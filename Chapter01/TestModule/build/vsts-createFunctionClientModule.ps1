...
if (-not $ModuleName) { $ModuleName = 'TestModule.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\TestModule'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\TestModule.psd1"
$prereqHash = @{
