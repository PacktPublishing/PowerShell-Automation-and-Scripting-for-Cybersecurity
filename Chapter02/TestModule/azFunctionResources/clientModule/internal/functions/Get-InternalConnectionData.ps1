...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'TestModule.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'TestModule.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'TestModule.Client.ProtectedToken'
